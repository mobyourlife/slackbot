...
function ProtoBot () {
   ...
   */
  self.action = function (verb, predicate, description, action) {
  ...
  function DoSomething (what) {
    let it = self.body.filter((i) => {
      console.log(i)
      return i.verb === what.action && i.predicate === what.terms
    })
  ...
  self.teach('ajuda', function () {
    if (self.body.length > 0) {
      ...
      for (var i of self.body) {
        sb += ` * \`${i.verb} ${i.predicate}\`: ${i.description}\n`
      }
  ...
  self.action('consultar', 'status', 'verificar os serviços que estão funcionando e os que não estão', actions.consultarStatus)
  self.action('criar', 'site', 'efetuar a criação de um novo site', actions.estouAprendendo)
  self.action('atualizar', 'site', 'força a atualização de um site específico', actions.estouAprendendo)
  self.action('atualizar', 'todos sites', 'força a atualização de todos os sites', actions.estouAprendendo)
}
